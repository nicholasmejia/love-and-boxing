...
- **Reaction State** — Post-answer beat that runs inside the existing Riddle Gap. K-confirm triggers a punch choreography. The pre-impact-frame is shared across all three outcomes: the right glove tweens to the chosen card (~150ms, `swing` SFX), the picked card flashes white on impact (`opponent_punch_body` + `menu_option_select` SFX), the body area renders the picked `DialogueAnswer.reaction_text` via the same typewriter used for prompt bodies, and `answer_submitted(outcome, picked_answer)` emits so gameplay can fire the outcome SFX and queue the next phase. Carousel input (`menu_left` / `menu_right` / `menu_confirm`) is locked the moment K fires (`_is_punching` gate) and stays locked until the per-outcome card choreography completes. The carousel ALSO fires `answer_committed(outcome)` synchronously on K-press — before the ~150ms glove travel that delays `answer_submitted` — so gameplay can end the Simon phase the instant the player commits. Gameplay's `_on_answer_committed` bumps `_gap_generation` (kills any in-flight breather/fresh gap that would otherwise re-fire `_defense.start()` during the upcoming attack), bumps `_defense_step_generation` (invalidates lingering `_on_step_blocked`/`_on_damage_taken` await handlers), calls `_defense.stop()`, and runs `_snap_clear_simon_visuals()`. Without this earlier tear-down, the K→impact 150ms window lets defense `step_flashed` collide with the attack telegraph and lets a repeat-phase timeout fire `_fail_with_damage` mid-commit — resetting the combo before attack even begins and scheduling a stale defense start that fires in parallel with the active attack phase. The card choreography itself branches on outcome:
  - **RIGHT** — the two unpicked cards animate out (slide to off-screen wrap positions + fade to transparent) and the picked card flies toward the opponent's body (~200ms, scale 1.0→0.4, alpha 1.0). When the picked card lands (~350ms after K), `card_struck_opponent(direction)` emits → gameplay calls `Opponent.set_action(HIT_LOW, Direction.RIGHT)` (mirrored to face the card coming from screen-right), holds `HIT_HOLD_DURATION` (~250ms), then transitions to `GUARD_DOWN`.
  - **NEUTRAL** — the two unpicked cards animate out and the picked card flies to the opponent's body identically to RIGHT (same flight tween, same flight-end `opponent_punch_body` SFX). On landing, the picked card runs a **Card Rebound** (see UI Visual Behavior). `card_struck_opponent` does NOT emit on this path — the opponent stays in `Action.IDLE` (Idle Bob continues uninterrupted) through the rebound; gameplay's NEUTRAL branch handles the prompt re-display via the existing `MatchPacing.NEUTRAL_READ_HOLD` flow.
  - **WRONG** — neither the side-card exit tween nor the picked-card flight runs. All three cards run a **Card Toss** (see UI Visual Behavior) in parallel, fanning outward. No card lands on the opponent, so `card_struck_opponent` does NOT emit and the flight-end `opponent_punch_body` SFX is suppressed (no flight). The opponent's reaction is owned entirely by gameplay's existing WRONG branch (`riddle_wrong` SFX, opponent swing-pose at the player, heart damage, screen-flash), which fires from the impact-frame `answer_submitted` emit and runs in parallel with the Card Toss.

  Tofu's deck (image-only) skips the reaction typewriter on all three paths — gameplay calls `RiddleBox.hide()` instead of `RiddleBox.show_reaction(reaction_text)` — but the per-outcome card choreography still plays. On the WRONG path, Reaction State persists through the breather gap until the next prompt loads. On the NEUTRAL path, gameplay waits for the reaction typewriter to complete (via the `body_render_complete` signal), then holds `MatchPacing.NEUTRAL_READ_HOLD` seconds, then re-displays the SAME prompt instantly via `RiddleBox.display_instant()` — no typewriter, no read floor — and replays the Simon chain at its current length from step 0. On the RIGHT path, it persists into the Attack Phase show/repeat until the first registered W/A/S/D press, which hides the riddle for the rest of the phase; an attack timeout leaves the reaction visible through `_return_to_defense` until the next prompt loads.
...
### Answer Card Animations (per outcome)

Per-outcome card choreographies inside Reaction State. The pre-impact-frame (glove launch, picked-card white flash, `answer_submitted` emit) is shared; these animations are what plays *after* the impact frame.

- **Card Toss** — The WRONG-outcome 3-card animation. All three answer cards translate along outward arcs the same way `WasdPrompt` Hit Toss tosses a successful attack prompt: scale tapers, opacity fades to 0, Z-rotation tumbles in the toss direction. Left card hooks left, picked (center) card jabs upward, right card hooks right; per-card jitter on apex height (~±15%) and toss duration (~±10%) keeps the three cards from moving identically. Replaces *both* the side-card exit tween and the picked-card flight on WRONG — the side cards do not run their normal slide-fade exit on this path.
- **Card Rebound** — The NEUTRAL-outcome picked-card animation. Plays *after* the picked card lands on the opponent's body (so the regular flight tween runs first, identical to RIGHT). Parabolic bounce arcing screen-right + down past the opponent, exiting off-screen over ~400ms. Z-rotation spins 720° clockwise across the same window. Scale tapers 0.5×→0.35×; alpha fades 1.0→0 over the trailing ~150ms. Reads as "deflected without effect." The side cards' regular exit tween runs in parallel (unchanged from RIGHT).
